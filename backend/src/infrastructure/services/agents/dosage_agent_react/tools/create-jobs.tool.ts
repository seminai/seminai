...
import { formatJobsTableMarkdown, type UnitMeta } from './format-jobs-table';
...
import {
  validateAgronomicPlan,
  type AgronomicViolation,
...
function buildUnitMetaMap(inputUnits: unknown): Map<string, UnitMeta> {
...
function compactViolation(violation: AgronomicViolation): Record<string, unknown> {
  return {
    code: violation.code,
...
function buildAgronomicGateBlockedResult(blocking: ReadonlyArray<AgronomicViolation>): string {
