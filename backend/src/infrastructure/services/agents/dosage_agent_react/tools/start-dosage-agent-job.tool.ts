/**
 * Tool: start_dosage_agent_job
 * Validates collected planning data from working memory and launches the
 * full dosage_agent pipeline as an async queue job.
...
const VALID_STRATEGIES: readonly string[] = ['min', 'max', 'avg', 'current'];
const VALID_OBJECTIVES: readonly string[] = [
  'minimize_interventions',
  'maximize_coverage',
  'balanced',
  'cost_effective',
];
const VALID_INTENSITIES: readonly string[] = ['low', 'medium', 'high'];
...
interface SelectedProductInput {
  productName: string;
  registrationNumber?: string;
  quantity?: number;
  quantityUnitOfMeasure?: string;
...
interface UnitInput extends JobUnitSource {
  id: string;
}
...
function normalizeTextForQuantity(value: string): string {
  return value
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/[®™]/g, '')
...
function containsProductPhrase(text: string, productName: string): boolean {
...
function selectProductsFromRequests(params: {
  availableProducts: readonly ProductInput[];
...
function inferRequestedProductsFromText(
  availableProducts: readonly ProductInput[],
  text: string | null,
...
function isApprovalSyntheticMessage(content: string): boolean {
...
async function loadLatestUserPlanningMessage(threadId: string): Promise<string | null> {
  try {
...
function buildInputDosageAgent(params: {
