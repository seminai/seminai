...
import { z } from 'zod';
import { llmMatchAgronomicNames, llmFilterMatchingNames } from '../shared/llmAgronomicMatcher';
import {
  resolveProductCrops,
  describeSource,
...
/**
 * Creates a tool that checks which crops a list of products is authorized for.
 * Looks up labels in LabelExtraction DB first, then falls back to BDF getImpieghi.
 */
export const createCheckProductCropAuthorizationsTool = (): DynamicStructuredTool => {
  return new DynamicStructuredTool({
    ...
    description:
      ...
      'Usa questo strumento quando l\'utente chiede quali prodotti a magazzino sono adatti a una specifica coltura (es. "quali prodotti sono per il melo?"). ' +
      'Accetta una lista di prodotti (nome + numero di registrazione) e una coltura target opzionale per filtrare.',
    schema: z.object({
      products: z
        .array(
          z.object({
            productName: z
              .string()
              .describe('Nome commerciale del prodotto (es. "Captano 80 WDG")'),
            registrationNumber: z
              .string()
              .optional()
              .default('')
              .describe('Numero di registrazione ministeriale / SIAN (es. "15549")'),
          }),
        )
        .min(1)
      ...
      targetCrop: z
        ...
        .optional()
        .describe(
          'Coltura target per filtrare (es. "melo", "vite", "olivo"). Se omessa, restituisce tutte le colture autorizzate per ogni prodotto.',
        ),
    }),
