...
import type { TokenCostsResponse } from './settings-costs-section.part-01-token-usage';
import { getModelColor } from './settings-costs-section.part-01-token-usage';
import {
  SectionState,
  buildMonthlyData,
} from './settings-costs-section.part-03-build-monthly-data';
...
export function SettingsCostsSection() {
  ...
  const costsQuery = useQuery({
    ...
    queryFn: () => customFetch<TokenCostsResponse>({ url: '/get-token-costs', method: 'GET' }),
  });

  const allUsages = useMemo(() => costsQuery.data?.data?.usages ?? [], [costsQuery.data]);

  const companies = useMemo(
    () => [...new Set(allUsages.map((u) => u.companyName).filter(Boolean))] as string[],
    [allUsages],
  );
  const models = useMemo(() => [...new Set(allUsages.map((u) => u.model))], [allUsages]);
                  ...
                  {filtered.map((u) => (
                    <TableRow key={u.id}>
                      ...
                      <TableCell className="text-right">
                        {u.promptTokens.toLocaleString('it-IT')}
                      </TableCell>
                      <TableCell className="text-right">
                        {u.completionTokens.toLocaleString('it-IT')}
                      </TableCell>
                      <TableCell className="text-right">
                        {u.totalTokens.toLocaleString('it-IT')}
                      </TableCell>
